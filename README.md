...
Raycasting от первого лица по сетке 64x64: по лучу на столбец экрана,
высота стены берётся из расстояния до плоскости камеры, поэтому коридоры
остаются прямыми.

- `UP`/`DOWN` — вперёд/назад, `LEFT`/`RIGHT` — поворот
- `SHIFT` + `LEFT`/`RIGHT` — шаг вбок
- `MODE` или `STOP` — пауза
- Меню: угол обзора, HUD, `Respawn`
