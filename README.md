...
- Два независимых места, включаются раздельно:
  ...
  - `Vpp shows DC` (по умолчанию включено) — если луч встал в **линию**, слот
    с `Vpp` показывает не размах, а сам уровень, и называется `Vdc`. Vpp на
    ровном напряжении — это шумовая полоса в пару отсчётов АЦП: верный ответ на
    вопрос, которого никто не задавал, а человеку, который смотрит на прямую
    линию, нужен её уровень. Порог — размах меньше четверти деления (6 отсчётов,
    с гистерезисом, чтобы слово в баре не мигало). **Только в баре**: панель и
    расставленные виджеты — это списки величин, выбранных поштучно, и список,
    у которого элементы меняют смысл, списком быть перестаёт
