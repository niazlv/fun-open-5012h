...
| **Измерения** | Vpp, частота (счётчиком и по спектру — независимо), скважность, Vrms, Vavg, тип сигнала, КНИ, джиттер |
...
**А свёртку не поймать в принципе.** Соблазнительная идея — снять запись на
двух частотах дискретизации и посмотреть, уехала ли частота, — на этом железе
не работает, и это доказуемо. Любая доступная частота есть `125/(PSC+1)`, то
есть **делит 125 нацело**; значит 125 — общее кратное любой пары, а
отображение `x → 125−x` сохраняет свёртку при каждой из них. 108 МГц читаются
как 17.0 и на 125, и на 62.5, и на 41.67, и на 31.25 Мвыб/с. Пара «сигнал и
его алиас» неразделима ни при каком числе частот — для этого нужен такт, не
являющийся целым делителем 125, то есть другая настройка ФАПЧ, а она утянет
за собой тайминги LCD и счёт времени.
