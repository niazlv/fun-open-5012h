...
```bash
make            # сборка, без ассетов DOOM — образ ~344 КБ из 512 КБ
...
Пакет ассетов DOOM — это ~208 КБ: больше половины образа ради одного
приложения. Поэтому по умолчанию он **не** линкуется, и
...
**`DOOM=1` пока не собирается**, но перебор стал вчетверо меньше. Прошивка без
ассетов занимает ~344 КБ, пакет — ещё ~208 КБ, вместе ~552 КБ при 512 КБ
флеша: не хватает ~40 КБ вместо прежних ~122. Освободившийся сектор конфига
закрыл большую часть разрыва. Правильный выход — не ужимать движок, а положить
пакет туда же, на SPI-флешку, где под него свободно почти 8 МБ. Это следующая
работа.
