...
При включении прибор сразу открывает осциллограф — это прибор, а список
приложений по дороге к нему не нужен. `SHIFT+MENU` выходит из осциллографа в
лаунчер, как из любого приложения, а `MENU > General Settings > Startup`
переключает поведение на `App Menu`, если список нужнее.
...
| **Как их видно** | полоса внизу или **виджеты, расставленные вручную** в редакторе раскладки, три размера вплоть до 16x32; шрифт и фон панели, два крупных слота в статусной строке; текст меню — двух размеров |
...
Прибор включается в осциллограф, так что этот экран — то, что показывает
`SHIFT+MENU`. Кому нужен список сразу: `MENU > General Settings > Startup`.
...
| `1X/10X` **удержание** | `SHIFT` — как и было: модификатор работает, пока клавиша нажата |
| `1X/10X` **одиночный тап** | включить/выключить щуп (см. [ниже](#щуп)) — то, что написано на клавише |
| `F1` / `F2` | быстро сменить левое / правое крупное показание в статусной строке |
...
- **Период и его половины.** `T` — время одного целого цикла, то самое, чего
  ...
  противоречить
- **Цвет показания говорит, можно ли ему верить.** Не «хороший сигнал или
  плохой» — этого прибор знать не может: 1% джиттера на тактовом генераторе это
  авария, а на серве норма. Зато он **точно** знает, когда его собственная
  запись не выдерживает того числа, которое он только что напечатал.

  | Цвет | Что случилось |
  | :--- | :--- |
  | **красный** | луч упёрся в АЦП — любое напряжение с такой записи это нижняя граница, а не измерение |
  | **красный** | периоды не согласуются друг с другом — частота, скважность и джиттер посчитаны по шуму |
  | **красный** | меньше 3 выборок на период: тут счётчик умеет показать ровно половину истинной частоты, и по записи это неотличимо |
  | **оранжевый** | показание меньше одного деления — один отсчёт АЦП составляет 4% от него |
  | **оранжевый** | в записи меньше пяти периодов |
  | **оранжевый** | джиттер на уровне пола интерполяции: это развёртка, а не сигнал |

  В каждом случае лечится одной клавишей: диапазон грубее, развёртка быстрее,
  запись длиннее. Настройки, показанные как величины (В/дел, с/дел, уровень
  триггера, частота дискретизации), не красятся никогда — они не измерения.
  Легенда есть в приборе: `MENU → Help → Scope Functions`

- **Джиттер — два числа в одном слоте**, `J  12ns~ 48ns`: слева σ (СКО
  периода), справа размах (самый длинный период минус самый короткий), а
  тильда их просто разделяет. Считается по **каждому** периоду записи, момент
  пересечения интерполируется до 1/256 выборки, поэтому σ осмысленна далеко
  ниже одного отсчёта

  <details>
  <summary>Как этим пользоваться: пол измерения, эталон, что плохо</summary>

  **Зачем два числа.** σ усредняет: один укороченный такт в записи из тысячи в
  ней растворится, а именно он и роняет систему — он живёт в размахе. Размах
  сам по себе не говорит, разовая это аномалия или так всё время; это говорит
  σ. Поэтому первое, что читается, — их **отношение**:

  | размах / σ | Что это |
  | :--- | :--- |
  | 3…5 | обычный случайный джиттер, выбросов нет |
  | > 8…10 | в записи сидит одна аномалия: рант, пропущенный фронт, помеха |
  | ≈ период | счётчик ловит не те пересечения — шум на среднем уровне или пологий фронт |

  **Сначала измерьте пол.** Прибор не может показать джиттер меньше своего
  собственного, а тот определяется не электроникой, а **периодом выборки**:
  фронт попадает между двумя отсчётами, и разрешение по времени выходит
  примерно `период выборки / 200`.

  | Период выборки | Ниже этого мерить нечем |
  | :--- | :--- |
  | 8 нс (полная скорость) | ~40 пс |
  | 100 нс | ~0.5 нс |
  | 1 мкс | ~5 нс |
  | 100 мкс | ~0.5 мкс |

  Отсюда главный практический признак: **замедлите развёртку вдвое.** Если σ
  выросла — вы меряете пол измерения, а не сигнал. Если не изменилась — это
  сигнал.

  **Эталон** — любой источник, тактируемый кварцем: выход таймера
  микроконтроллера, кварцевый генератор, линия UART от аппаратного модуля. Их
  собственный джиттер — пикосекунды, то есть заведомо ниже того, что этот
  прибор видит, поэтому показание на них **и есть ваш пол** на этой развёртке.
  Не эталон: 555, RC-генераторы, программный ШИМ, сеть 50 Гц.

  Померив пол, вычитайте его квадратично: `σ_сигнала = √(σ_показ² − σ_пол²)`.

  **Порядок величин, к которым стоит привыкнуть** (σ в долях периода):

  | Источник | σ / T |
  | :--- | :--- |
  | кварцевый генератор, аппаратный таймер | ниже пола прибора |
  | ШИМ с аппаратного таймера МК | ниже пола |
  | программный ШИМ, бит-бэнг | 0.01…1%, и растёт от прерываний |
  | сервопривод, ИК-приёмник | 0.1…1% |
  | RC-генератор, 555 | 0.1…1% |
  | импульсный преобразователь с dithering | большая σ **намеренно** |

  **Как мерить, чтобы число было честным**

  1. Развёртка — самая быстрая, на которой в записи ещё десятки периодов.
  2. Сигнал на весь экран: пол обратно пропорционален амплитуде.
  3. Фронт покруче. На пологом фронте амплитудный шум прямо превращается в
     джиттер: `σ_t ≈ время фронта × (шум / амплитуда)` — вы померите свой шум.
  4. Рядом поставьте `T`: 4 мкс на периоде 20 мс и на периоде 40 мкс — это
     0.02% и 10%, то есть разные вселенные. Без периода число не значит ничего.
...
- Два независимых места, включаются раздельно:
  ...
  - `Vpp shows DC` (по умолчанию включено, переключается **удержанием AC/DC**)
    — если вход **в DC** и луч встал в линию, слот с `Vpp` показывает не размах,
    а сам уровень. Метка при этом остаётся вольтовой: в баре у `V` появляется
    значок постоянного тока под ней, а при крупном тексте слово становится
    `Vdc`. Vpp на ровном напряжении — это шумовая полоса в пару отсчётов АЦП:
    верный ответ на вопрос, которого никто не задавал, а человеку, который
    смотрит на прямую линию, нужен её уровень
  - В **AC** подмены нет никогда: постоянную составляющую там снимает железо до
    преобразователя, показывать нечего, и `Vpp` — честное показание
  - Порог — размах меньше четверти деления (6 отсчётов АЦП, возврат на 10, чтобы
    надпись не мигала). **Только в баре**: панель и расставленные виджеты — это
    списки величин, выбранных поштучно, и список, у которого элементы меняют
    смысл, списком быть перестаёт
...
| | `Probe` | какой щуп на входе: `1x` … `1000x` (см. [ниже](#щуп)) |
| | `Probe applied` | учитывать его или нет — то же, что тап по `1X/10X` |
...
| | `Size` | `Small` (6x8, ~6 показаний) / `Large` (8x16, 4) / `Huge` (16x32, одно) |
| | `Background` | `Dim` / `Solid` / `None` |
| | галочки | какие из 17 величин показывать в полосе |
| `Status line` | `Left`, `Right` | что стоит в двух крупных слотах, по отдельности |

Слот и виджет выбирают из 21 величины, а не из 17: к измерениям добавлены четыре
**показания баров** — `V/div`, `s/div`, уровень триггера и частота дискретизации.
Они читаются из настроек, а не из записи, поэтому есть и до первого захвата, и на
остановленной развёртке. В полосе их нет намеренно: полоса — это строка
измерений, а эти четыре и так на экране в пяти пикселях от неё. Смысл в том, что
их можно **поставить** — см. [крупный текст](#крупный-текст).

Два крупных слота меняются и без меню: **`F1`** открывает над статусной строкой
одну строку — левый слот, **`F2`** — правый. Стрелки сразу листают величину (окно
открывается уже в режиме правки, значение применяется на месте), `MENU` закрывает.
Это та же строка меню, что в `Status line`, а не её копия, так что разойтись в
названиях они не могут.

### Щуп

Клавиша с надписью `1X/10X` на этом железе физически и есть `SHIFT` — один пин,
один бит. Модификатор используют **удержанием**, поэтому одиночный тап
(нажал-отпустил, без аккорда, короче 400 мс) остался свободен и делает ровно то,
что написано на клавише: включает и выключает щуп. Удержание — по-прежнему
`SHIFT`, двойной тап — по-прежнему залипание, и его два переключения щупа взаимно
гасятся, так что раскладка клавиш не изменилась ни в чём.

Какой щуп — говорится в меню, потому что клавиша угадать не может: пассивные
бывают 1x и 10x, делители-пады дают 2x, 5x, 20x и 50x, токовые и
дифференциальные — 25x, 200x, 500x, высоковольтные — 100x и 1000x. Список ровно
такой, `1x` в нём — это «без пересчёта».

Что происходит при включении: **всё, что прибор сообщает**, умножается на
коэффициент — ярлык В/дел, измерения, курсоры, уровень триггера. Луч при этом не
двигается: щуп меняет то, что написано на оси, а не то, где стоит пятно, поэтому
множитель уходит и в милливольты-на-отсчёт, и в милливольты-на-пиксель
одновременно.

Видно это в левом нижнем углу: поле масштаба становится двухстрочным — сверху
коэффициент, снизу эффективные В/дел, оранжевым. Одного ярлыка не хватило бы:
`5 V` — это и 5 В/дел без щупа, и 500 мВ/дел через 10x, а с 25x и 100x в списке
такое уже не угадывается.

В режиме калибровки щуп принудительно выключен: калибровать собственный тракт
прибора через делитель — это калибровка делителя, а не прибора.

**Калибровка: порядок действий**

Всё, что прибор сообщает в вольтах, стоит на пяти измеренных числах. Пока
экземпляр не откалиброван, это числа **чужого** экземпляра — близкие, но на
несколько процентов мимо, и эти несколько процентов как раз и заметны.

| | Что делать | Зачем |
| :--- | :--- | :--- |
| 1 | Закоротить BNC (или снять щуп) | открытый вход 1 МОм — антенна, и наводка сорвёт все три измерения ниже |
| 2 | `Calibration → Auto-calibrate`, `MODE` | ~30 с без приборов: ноль (Z), рассогласование АЦП (D), шаг ЦАПа (O), изгиб тракта (N) |
| 3 | Подать известный уровень, набрать его `←`/`→`, `MODE` | усиление (S) этого диапазона |
| 4 | `SHIFT+↑`/`↓` — следующий диапазон, повторить; `STOP` — закончить | восемь диапазонов — восемь путей через аттенюатор, у каждого свой допуск |

Шаги 3–4 существуют и отдельно (`Calibration → Calibrate gain`) — когда
понадобился диапазон, до которого раньше не дошли руки, или появился источник.

Порядок не произвольный: **каждый шаг измеряет то, что следующий иначе впитает
в себя и переврёт**. Ноль первым — уровень, отмеренный от съехавшего нуля, съехал
на столько же, и подгонка усиления после этого не убрала бы ошибку, а
отмасштабировала её. Изгиб раньше усиления — усиление это один множитель, то
есть прямая, и на кривом тракте подгонка по 9 В выходит верной в 9 В и неверной
в 5 В. Усиление последним и по диапазонам.

Уровень везде вводится **на кончике щупа** — то число, которое показывает
мультиметр. Лучше брать уровень на три-четыре деления калибруемого диапазона:
один отсчёт АЦП — это 1/25 деления, и на уровне в одно деление квант составляет
4% от показания, то есть подгонка получается грубее ловимой ошибки.

Та же инструкция живёт в приборе: `MENU → Help → Calibration`.

**Нелинейность тракта** (шаг 4 автокалибровки)

Усиление — это одно число на диапазон, и одним числом описывается только
прямая. Тракт не прямой: усилитель поджимает на больших размахах, у
преобразователя своя INL. Отсюда эффект, который выглядит как неисправность
калибровки — подогнали 9.18 В точно, а 5.28 В уехало на процент в другую
сторону, потому что подгонка наклона компенсировала кривизну ровно в одной
точке.

Меряется это **без эталона вообще**. Смещающий ЦАП двенадцатибитный, АЦП —
восьми: шагаем ЦАПом по всему окну преобразователя с закороченным входом и
смотрим, где оказался луч на каждом шаге. Отклонение от прямой — это тракт,
потому что собственная нелинейность ЦАПа (±2 LSB из 4096) составляет 0.06
кода АЦП. Линейка в восемь раз точнее измеряемого — это линейка.

Из результата берётся только кривизна: постоянная часть — это ноль, у него свой
шаг, наклон — это усиление, у него свой. Оба преобразователя меряются в одном
проходе (кольцо чередует их) и кривые у них разные. Тридцать три шага, около
десяти секунд, и в конце прибор сообщает, сколько нашёл — в процентах от
четырёх делений.

Хранится это парой коэффициентов (`calib_nl2`) и применяется **в измерениях, не
в отрисовке**: полкода — это полпикселя, а просмотр таблицы на каждую выборку в
свипе стоит дороже, чем стоит эта половина. Поправка нелинейна, поэтому она
применяется к агрегатам точно, а не к среднему «примерно»: среднее правится
через второй момент, СКЗ — через третий, а пики правятся поштучно, как коды.
Симметричный сигнал вокруг нуля СКЗ при этом не меняет — квадратичный изгиб
выпрямляет такой сигнал в **среднее**, а не в амплитуду.

Порядок калибровки поэтому такой и только такой: **ноль → рассогласование АЦП →
шаг ЦАПа → изгиб → усиление по диапазонам**. Каждый шаг чинит то, что следующему
мешало бы измерять, и последний меряет уже выпрямленный тракт.

Проверено в эмуляторе с заведомо кривым трактом (`--afe nl2=3`, остальные
искажения выключены):

| Подано | До калибровки | После |
| :--- | :--- | :--- |
| 400 мВ | 405 мВ (+1.3%) | 398 мВ (−0.5%) |
| 800 мВ | 823 мВ (+2.9%) | 796 мВ (−0.5%) |

До — ошибка **растёт** с уровнем, это и есть изгиб. После — одинаковый процент
на обоих уровнях, то есть осталась чистая ошибка усиления, а её убирает
следующий шаг.

**Калибровка усиления по диапазонам** (`MENU → Calibration → Calibrate gain`)

Восемь диапазонов по вертикали — это восемь путей через аттенюатор, у каждого
свой делитель и свой допуск. `calib_vs_mult[]` — утверждение «столько
милливольт в одном отсчёте АЦП», и оно **своё на каждый диапазон**: 3% мимо на
500 мВ/дел не говорит ничего про 50 мВ/дел. Автокалибровка находит три
параметра из четырёх — нуль, рассогласование АЦП и шаг ЦАПа проверяются на
закороченном входе, потому что это утверждения о самом приборе. Усиление так
проверить нельзя: нужен образцовый уровень, а генератора на плате нет. Поэтому
шаг усиления в конце автокалибровки заканчивался словами «this range only», и
дальше оставалось только вводить число руками.

Этот проход — тот же шаг, но повторяемый: подали уровень, сказали какой,
`MODE` — диапазон подогнан, и прибор остаётся здесь же для следующего.

| Клавиша | Что делает |
| :--- | :--- |
| `←` `→` | сколько подано на самом деле: шаг 10 мВ, с удержанием — 200 мВ |
| `SHIFT+↑` `↓` | следующий диапазон, не выходя из прохода |
| `MODE` | измерить и подогнать этот диапазон |
| `STOP` | закончить (скажет, на скольких из восьми усиление теперь измерено) |

Строка над клавишами всё время показывает, что прибор читает, что вы ввели, и
расхождение в процентах: ошибку видно **до** того, как она применена, а после
— та же строка подтверждает результат. Один источник закрывает два-три
диапазона, на которые он влезает; на слишком мелком для него луч упирается в
край, и подгонка отклоняется, а не выдумывается.

Уровень — **на кончике щупа**, то есть то самое число, которое показывает
мультиметр рядом, а не то, что доходит до BNC. Хранится он всё равно в вольтах
на входе прибора, потому что `calib_vs_mult` — про прибор и не знает, что к
нему прицеплено. Обратная сторона: 10x щуп, чей собственный делитель врёт на
пару процентов, этой подгонкой вносит своё враньё в диапазон — если диапазоны
должны быть верны и без щупа, калибруйте на 1x.
...
| `F1` | размер этого показания: 6x8 → 8x16 → 16x32 → 6x8 |
...
Третий размер, `16x32`, — это удвоенный 8x16, а не отдельный шрифт: шрифта такого
кегля в образе нет, и он стоил бы ~3 КБ флеша на девяносто пять символов, тогда
как удвоение стоит умножения в композиторе. Зачем он вообще: это единственный
размер, который **не влезает ни в один из баров**, а значит — единственный ответ
на «показания в барах мелкие».

**Позиция считается от угла, а не от левого верхнего пикселя.** Показание
привязывается к тому из четырёх углов области луча, к которому оно ближе, и
хранится расстоянием до этого угла — привязка выбирается сама, при каждом
перемещении, спрашивать про углы никого не нужно. Отсюда два следствия. Показание,
которое выросло, остаётся там, где его поставили: при 6x8 оно 60 px шириной, при
16x32 — 160, и вся разница раньше вылезала вправо-вниз, так что показание в правом
нижнем углу уезжало за экран. Теперь оно растёт **внутрь**, туда, где место. И
раскладка переживает изменение геометрии: то, что отмерено от правого или нижнего
края, не надо переставлять, если то, от чего оно отмерено, сдвинулось. Раскладки,
сделанные раньше, читаются как раньше — «от левого верхнего» это привязка с
нулевым кодом.

### Крупный текст

Читаемость тут упирается в 320x240, поэтому настроек две, и делят они экран по
живому: одна отвечает за меню, другая — за показания прибора.

**Меню и текстовые страницы** — `MENU > General Settings > Menu Text`:
`Normal` / `Large`.

`Large` — это Terminus 8x16 вместо 6x8, строка 26 px вместо 20, окно 244 px вместо
180. Влезает девять строк вместо одиннадцати, дальше меню прокручивается — как и
раньше, когда пунктов было больше, чем места. Размер применяется к тому меню, из
которого его меняют: строка растёт под курсором, пока крутится значение.

Дальше 8x16 меню не растягивается, и это не лень: самая длинная подпись —
`Arrange layout...`, семнадцать символов, и справа в той же строке должно
поместиться значение (`Oscilloscope`, `? (above nyquist)`). Удвоенный 6x8 (12x16)
съел бы на 50% больше ширины при той же высоте и читался бы грубее; 16x32 в строку
не влезает вообще.

Текстовые страницы (`Help`, `Key Bindings`, `System Info`) крупнеют **с переносом
строк**: `lcd_puts` не переносит, а обрезает по краю панели, а страницы свёрстаны
под 46 колонок 6x8 — при 8 px на символ они теряли бы последние двенадцать.
Обрезанный диагностический вывод хуже мелкого, поэтому длинная строка переносится
и продолжается со сдвигом в 8 px. Прокрутка и разделы остаются по логическим
строкам, а полоса прокрутки и проценты считаются по **экранным** — на странице с
переносами это разные числа.

**Показания прибора** — `Menu > Measurements > Panel over the trace > Size`:
`Small` (6x8) / `Large` (8x16) / **`Huge` (16x32)**.

`Huge` — это удвоенный 8x16, а не отдельный шрифт: шрифта такого кегля в образе
нет, и он стоил бы ~3 КБ флеша, тогда как удвоение стоит умножения в композиторе.
Полоса при этом становится одной строкой вместо двух — две строки по 36 px забрали
бы 72 пикселя из 200, а полоса, занимающая треть экрана, уже не полоса. В строку
влезает 18 символов, то есть **одно** показание; сколько осталось за кадром,
говорит маркер (`+3`), который при нехватке места ужимается до двух символов, а не
съедает само показание.

Нужно несколько крупных — это редактор раскладки: по показанию в каждый угол.

**Почему бары не меняются ни от чего из этого.** Верхний бар 20 px, нижний 16 px, и
оба забиты от края до края: режим триггера кончается на x=41, индикатор захвата
занимает 46..77, карта записи начинается с 81, частота дискретизации и батарея
держат правый угол. Из 240 строк экрана 200 забирает сетка 10 делений по 25 px.
Чтобы в бар влез 16x32, нужно 34 px высоты — то есть отнять строки у сетки и либо
уменьшить шаг деления (минус 32% разрешения луча по вертикали), либо показывать 8
делений вместо 10 (минус 20% диапазона). Поэтому 8x16 — потолок баров, а ответ на
«в барах мелко» другой: **вынести значение из бара**. `V/div`, `s/div`, уровень
триггера и частота дискретизации есть в списке величин, редактор ставит их куда
угодно, `F1` доводит до 16x32. Композитор столбцов рисует их тем же проходом, что и
луч, так что они не мигают, а сетка под ними приглушается.
