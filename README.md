...
<details>
<summary><b>Чем платим за ассеты на SPI</b> — 50 fps против 18</summary>

**С паком в образе было ~50 fps, с текстурами на SPI-флешке стало 17–18.**
Это честная цена текущей реализации, и вот из чего она складывается.
...
Модель обещала ~2 мс на кадр. На приборе вышло около 35. Разрыв не объяснён до
конца, известные подозреваемые:

- **индексация кеша.** Слот выбирается как `(offset >> 4) & 63`, а колонки
  лежат в паке через ~64 байта — значит соседние колонки попадают в слоты
  через один, и работает фактически четверть кеша. Сдвиг на `>> 6` должен
  вернуть остальные три четверти;
- **такт SPI.** Сейчас PCLK/8 = 15.6 МГц с обычной командой `03h`. Команда
  Fast Read `0Bh` рассчитана на 104+ МГц, и PCLK/4 = 31 МГц для неё —
  щадящий режим. Это ровно вдвое;
- **накладные расходы на транзакцию.** Каждое чтение сейчас идёт байт за
  байтом с опросом флагов; DMA на приёме снял бы это с процессора целиком.

Ни одна из трёх не сделана — сначала хотелось увидеть работающую картинку и
настоящую цифру, а не оптимизировать по прогнозу.

Если частота кадров важнее места, соберите цельный пакет без `--split` и
уберите что-нибудь другое из образа: с паком во flash рендерер работает по
указателям и никакой шины между ним и текстурами нет.
