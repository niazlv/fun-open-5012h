...
- **Пользоваться**
  ...
  - [Доступные приложения](#доступные-приложения): [осциллограф](#1-oscilloscope) ·
    ...
    [Тетрис](#8-tetris) · [DOOM](#9-doom) · [дампы](#10-coredump-viewer) ·
    [flash](#11-flash-viewer)
- **Как это сделано**
...
- `Off` — обычная трасса
- `Infinite` — огибающая копится и **не уходит**: рант, мигнувший один раз
  полчаса назад, всё ещё на экране. Ровно то, ради чего накопление и нужно,
  когда ищешь редкое событие
- `Decay (CRT)` — та же огибающая, но с яркостью, которая кончается за
  ~1.5 с, и с краями, которые подтягиваются к живой трассе. Свежий след
  светится ярче постоянного тона и гаснет сквозь него; яркость намеренно
  ниже цвета луча — луч должен оставаться самым ярким на сетке
...
Что остаётся ярким — полоса, которую джиттер честно перерисовывает каждый
кадр. На настоящей трубке она такая же яркая по той же причине.
