...
static const char *const g_scope_help_lines[] =
{
  ...
  "",
  INFO_HEAD "The colour of a reading",
  "Readings are coloured by how much the",
  "RECORD supports them - not by whether the",
  "signal is any good. The instrument does not",
  "know what your signal is for; it knows",
  "exactly what its own record can carry.",
  "",
  "RED, a number the record cannot support:",
  "  the trace clipped, so every voltage off",
  "  it is a lower bound and not a measurement",
  "  the periods disagree with each other, so",
  "  frequency, duty and jitter are arithmetic",
  "  on noise",
  "  under 3 samples per period - where the",
  "  counter can read exactly half the true",
  "  frequency and nothing else says so",
  "",
  "ORANGE, supported but against a limit:",
  "  the reading is worth under one division,",
  "  so one ADC code is 4% of it",
  "  fewer than five cycles in the record",
  "  jitter at the interpolation floor, which",
  "  is the timebase and not the signal",
  "",
  "In every case the fix is a key: a coarser",
  "range, a faster timebase, a longer record.",
  "Settings shown as readings (V/div, s/div,",
  "trigger level, sample rate) are never",
  "coloured - they are not measurements.",
};
...
{
  INFO_HEAD "What this is for",
  "Every voltage on screen comes from five",
  "measured numbers. Until this unit has been",
  "calibrated they are ANOTHER unit's - close,",
  "but a few percent out, and the few percent",
  "is the part you would notice.",
  "",
  INFO_HEAD "Do this, in this order",
  "1. Short the BNC (or take the probe off).",
  "2. Calibration > Auto-calibrate, MODE.",
  "   About thirty seconds, no equipment. It",
  "   finds Z, D, O and N below.",
  "3. It stops and asks for a reference. Apply",
  "   a DC level you know the value of, type",
  "   that value with LEFT/RIGHT, MODE. That",
  "   is S, for the range on screen.",
  "4. SHIFT+UP/DOWN for the next range, and",
  "   the same again. STOP when done.",
  "",
  "Steps 3 and 4 are also Calibration >",
  "Calibrate gain on their own, for when a",
  "range is added later or a source turns up.",
  "",
  INFO_HEAD "Why that order",
  "Each step measures something the next one",
  "would otherwise absorb and get wrong.",
  "",
  "Zero before everything: a level measured",
  "against a zero that is off is off by the",
  "same amount, and a gain trim would then",
  "scale the error instead of removing it.",
  "",
  "The bend before the gain: gain is one",
  "multiplier and a multiplier is a straight",
  "line. On a bent chain, a trim aimed at 9 V",
  "comes out right at 9 V and wrong at 5 V -",
  "which is the symptom, not a broken trim.",
  "",
  "Gain last, and per range: eight ranges are",
  "eight paths through the attenuator, each",
  "with its own tolerance. One says nothing",
  "about the others.",
  "",
  INFO_HEAD "The parameters",
  "Z zero    - offset DAC centre. Shared.",
  "D delta   - ADC B against ADC A. Shared.",
  "O offset  - DAC step, per RANGE.",
  "N bend    - second-order nonlinearity, per",
  "            CONVERTER, shared by all ranges",
  "            because what bends is past the",
  "            attenuator. Measured, not typed.",
  "S scale   - gain, per RANGE. The one that",
  "            needs a reference voltage.",
  "",
  INFO_HEAD "How the bend is measured",
  "With nothing connected, and no calibrator:",
  "the offset DAC is 12 bits and the converter",
  "is 8, so stepping the DAC across the window",
  "measures the chain against a ruler eight",
  "times finer than itself. What departs from",
  "a straight line is the amplifier and the",
  "converter. Both converters are measured -",
  "they do not have the same curve - and the",
  "run reports what it found as a percentage",
  "of four divisions.",
  "",
  INFO_HEAD "About the reference",
  "The value you type is at the PROBE TIP -",
  "the number on the meter beside you, not",
  "what reaches the BNC.",
  "",
  "Best on a level that fills three or four",
  "divisions of the range being calibrated:",
  "one ADC code is 1/25 of a division, so on",
  "a level of one division a code is 4% of",
  "the reading and the trim is coarser than",
  "the error being chased.",
  "",
  "A 10x probe whose own ratio is off by a",
  "few percent puts that error into the range",
  "it is calibrated on. Calibrate at 1x if",
  "the ranges have to be right without it.",
  "",
  INFO_HEAD "When it refuses",
  "Clipping - the level does not fit this",
  "range. Take a coarser one.",
  "",
  "Reading too small or wrong polarity - less",
  "than a division of deflection, or the leads",
  "are the other way round.",
  "",
  "Not a trim - the correction came out over",
  "50%, which is a wrong range or a mistyped",
  "level, not a calibration.",
  "",
  "Input is not quiet - the linearity sweep",
  "saw more scatter than a converter has. An",
  "open 1 MOhm input is an antenna; short it.",
  "",
  INFO_HEAD "Keys on the calibration screen",
  "TRIG_UP/DN   - change the value",
  "SHIFT+TRIG_x - next parameter",
  "SHIFT+L/R    - timebase, SHIFT+U/D - volts",
  "AUTO         - the run described above",
  "MODE         - hide the hint",
  "",
  "The readout turns RED when the parameter",
  "does not belong to the current mode.",
  "Calibration > Enter values types the same",
  "numbers with a keypad instead, and Reset",
  "calibration puts the factory ones back -",
  "another unit's, not this one's.",
};
